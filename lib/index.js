...
function grep (manifests, options) {
  ...
  included.forEach((pattern) => {
    ...
    files.forEach((file) => {
      ...
      // Replace the old version number with the new version number => delete 'g' option, replace only first occurence
      let oldVersionPattern = new RegExp(oldVersion.replace(/\./g, '\\.'));
